...
metadata
{
	definition (name: "Aeon Multisensor 6 Gen5 (Configurable)", namespace: "john.fullman", author: "john.fullman@gmail.com")
    {
    ...
    preferences
    {
        ...
        input "MotionSensitvity", "number", title: "How sensitive should the motion sensor be (%):", required: true, displayDuringSetup: true, defaultValue: 50
        ...
        input "ReportUltraviolet1", "bool", title: "Report ultraviolet in Group 1?", required: true, displayDuringSetup: true, defaultValue: true
        ...
        input "ReportUltraviolet2", "bool", title: "Report ultraviolet in Group 2?", required: true, displayDuringSetup: true, defaultValue: true
	...
	simulator
    {
		...
		for (int i = 0; i <= 11; i += 1) {
			status "ultraviolet ${i}": new physicalgraph.zwave.Zwave().securityV1.securityMessageEncapsulation().encapsulate(
				new physicalgraph.zwave.Zwave().sensorMultilevelV2.sensorMultilevelReport(scaledSensorValue: i, sensorType: 27)
			).incomingMessage()
		}
	...
	tiles
    {
		...
		valueTile("ultraviolet","device.ultraviolet",inactiveLabel: false) {
            state "ultraviolet",label:'${currentValue} UV',unit:"UV"
        }
		standardTile("touch","device.touch") {
...
def zwaveEvent(physicalgraph.zwave.commands.configurationv1.ConfigurationReport cmd) {
    ...
    {
    	case 2: //wakeup when repowered
            if(decval == 0)
            {
                log.info "Wake-up on repower is disabled."
            }
            else
            {
                log.info "Wake-up on repower is enabled."
            }
            break
        ...
        case 5: //motion commandset
        	...
        	break
        case 6: //motion sensitivity
            log.info "Motion sensitivity is ${decval / 127.0}%"
            break
        case 7: //touch enabled
        	if(decval == 1)
            {
            	log.info "Touch sensor is enabled."
            }
            else
            {
            	log.info "Touch sensor is disabled."
            }
        	break
        ...
        case 44: //battery threshold
        	...
        	break
        case 46: //low temp warning enabled
        	if(decval == 1)
            {
            	log.info "Low temperature warning is enabled."
            }
            else
            {
            	log.info "Low temperature warning is disabled."
            }
        	break
...
def configure()
{
    ...
    int iMotionSensitvity = (MotionSensitvity * 1.27).toInteger()
    ...
    //configure report intervals
		...
		//motion sensitivity
		zwave.configurationV1.configurationSet(parameterNumber: 6, size: 1, scaledConfigurationValue: iMotionSensitvity),
		...
		zwave.configurationV1.configurationSet(parameterNumber: 7, size: 1, scaledConfigurationValue: 1)
    ]
    
    setConfigured()
	
    //return secureSequence(request) //+ ["delay 20000", zwave.wakeUpV1.wakeUpNoMoreInformation().format()]
    return secureSequence(request, 3000)
}
...
def GetSensorUpdates()
{
	return [
		...
		zwave.sensorMultilevelV5.sensorMultilevelGet(sensorType: 27), //ultraviolet
...
def GetCurrentConfig()
{
	return [
		...
		zwave.configurationV1.configurationGet(parameterNumber: 46),
